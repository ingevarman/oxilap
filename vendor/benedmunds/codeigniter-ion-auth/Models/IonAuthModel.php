...
class IonAuthModel
{
	...
	public function getUsersGroups(int $id=0)
	{
		...
		//garmaser se reemplazo [$this->tables['groups'] . '.description] por [$this->tables['groups'] . '.*'] para mostrar todos los campos de group
		return $builder->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.*')
